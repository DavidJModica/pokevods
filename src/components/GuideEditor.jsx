...
const GuideEditor = () => {
  ...
  const fetchGuide = async () => {
    try {
      ...
      const response = await fetch(`/api/hosted-guides?id=${id}`, {
        headers: {
  ...
  const saveDraft = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const url = id ? '/api/hosted-guides' : '/api/hosted-guides';
      ...
      // If creating new guide, navigate to edit mode
      if (!id) {
        navigate(`/admin/guides/edit/${result.guide.id}`);
      }
  ...
  const validateGuide = () => {
    ...
    for (let i = 0; i < guideData.sections.length; i++) {
      ...
      if (section.type === 'matchups' && section.matchups.length === 0) {
        alert(`Matchups section "${section.title}" should have at least one matchup`);
        setActiveTab(i);
        return false;
      }
  ...
  return (
    <div className="guide-editor-container">
      <div className="guide-editor-header">
        <h1>{id ? 'Edit Guide' : 'Create New Guide'}</h1>
